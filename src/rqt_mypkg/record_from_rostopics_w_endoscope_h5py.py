...
camera_names = ['img_l', 'img_r', 'img_lw', 'img_rw']
usb_image_left = None
...
def data_saver_h5py(queue):
  while True:
    item = queue.get()
    if item is None:
      break  # None is our signal to stop
    save_path, ee_points, imgs_l, imgs_r, imgs_lw, imgs_rw = item
    compression_opts = 5
    save_len = len(ee_points)
    with h5py.File(save_path + '.hdf5', 'w', rdcc_nbytes=1024**2*2) as root:
      _ = root.create_dataset('img_l', (save_len, 480, 640, 3), dtype='uint8',
                                    chunks=(1, 480, 640, 3), compression='gzip',compression_opts=compression_opts)
      _ = root.create_dataset('img_r', (save_len, 480, 640, 3), dtype='uint8',
                                    chunks=(1, 480, 640, 3),  compression='gzip',compression_opts=compression_opts)
      _ = root.create_dataset('img_lw', (save_len, 480, 640, 3), dtype='uint8',
                                    chunks=(1, 480, 640, 3),  compression='gzip',compression_opts=compression_opts)
      _ = root.create_dataset('img_rw', (save_len, 480, 640, 3), dtype='uint8',
                                    chunks=(1, 480, 640, 3),  compression='gzip',compression_opts=compression_opts)
...
# Start worker thread
worker = threading.Thread(target=data_saver_h5py, args=(image_queue,))
worker.start()
...
while(True):
  ...
  if isRecord:
    ...
    imgs_l.append(cv2.resize(usb_image_left, (640, 480), interpolation = cv2.INTER_LINEAR))
    imgs_r.append(cv2.resize(usb_image_right, (640, 480), interpolation = cv2.INTER_LINEAR))
    imgs_lw.append(cv2.cvtColor(endo_cam_psm2, cv2.COLOR_BGR2RGB))
    imgs_rw.append(cv2.cvtColor(endo_cam_psm1, cv2.COLOR_BGR2RGB))

    # image_queue.put((save_name_left, cv2.cvtColor(usb_image_left, cv2.COLOR_BGR2RGB)))
    # image_queue.put((save_name_right, cv2.cvtColor(usb_image_left, cv2.COLOR_BGR2RGB)))
    ...
    if requiresSaveCsv is True:
      ...
      save_path = os.path.join(ep_dir, "img_data")
      image_queue.put((save_path, ee_points, imgs_l, imgs_r, imgs_lw, imgs_rw))
